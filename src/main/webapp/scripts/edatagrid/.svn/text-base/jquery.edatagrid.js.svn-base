		...
		$(document)
				...
				.bind(
						...
						function(e) {
							var p = $(e.target)
									.closest(
											'div.datagrid-view,div.combo-panel,div.window,div.window-mask');
							if (p.length) {
										...
										opts,
										{
											...
											onAfterEdit : function(index, row) {
												...
												if (url) {
													...
													for ( var i = 0; i < fields.length; i++) {
														...
														var col = $(this)
																.edatagrid(
																		...
																		field);
														if (col.editor
																&& opts.originalRow[field] != row[field]) {
															changed = true;
															break;
														}
													}
													...
													opts.onSave.call(target,
															index, row);
												}
												if (opts.onAfterEdit)
													opts.onAfterEdit.call(
															target, index, row);
											},
											onCancelEdit : function(index, row) {
												...
												if (opts.onCancelEdit)
													opts.onCancelEdit.call(
															target, index, row);
											},
											onBeforeLoad : function(param) {
												if (opts.onBeforeLoad.call(
														target, param) == false) {
													return false
												}
